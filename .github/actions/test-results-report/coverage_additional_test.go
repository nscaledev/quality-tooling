...
func TestConfigParsingCoversOverridesAndEnvironment(t *testing.T) {
	env := map[string]string{
		...
		"INPUT_COMPONENT_NAME":                    "uni-region",
		"INPUT_COMPONENT_VERSION":                 "v1.2.3",
		"INPUT_COMPONENT_REF":                     "v1.2.3",
		"INPUT_COMPONENT_REPO":                    "nscaledev/uni-region",
		"INPUT_COMPONENT_VERSION_URL":             "https://region.example/api/version",
		"INPUT_COMPONENT_VERSION_TOKEN":           "component-token",
		"INPUT_COMPONENT_VERSION_TIMEOUT_SECONDS": "6",
	...
	if config.ComponentName != "uni-region" || config.ComponentVersion != "v1.2.3" || config.ComponentRef != "v1.2.3" || config.ComponentRepo != "nscaledev/uni-region" || config.ComponentVersionURL != "https://region.example/api/version" || config.ComponentVersionToken != "component-token" || config.ComponentVersionTimeout != 6*time.Second {
		t.Fatalf("unexpected component metadata config: %+v", config)
	}
...
func TestMainHelpersCoverOutputAndPathBranches(t *testing.T) {
	...
	if err := writeOutputs(outputPath, analysis, true, ComponentMetadata{Name: "uni-region", Version: "v1.2.3", Ref: "v1.2.3", Repo: "nscaledev/uni-region"}); err != nil {
