	...
	Context("When rendering input for AI analysis", func() {
		Describe("Given Claude is asked to write the report section", func() {
			It("should ask for pattern-level triage instead of repeated raw test lists", func() {
				...
				Expect(prompt).To(ContainSubstring("Use Grafana observations, CR observations, and OpenStack observations only as supporting evidence"))
				...
				Expect(prompt).To(ContainSubstring("Do not add a separate OpenStack section"))
				...
				Expect(prompt).To(ContainSubstring("OpenStack/Neutron showed IpAddressGenerationFailure on the external network"))
				Expect(prompt).To(ContainSubstring("OpenStack/Neutron showed SubnetInUse for the subnet backing a stuck Network CR"))
				Expect(prompt).To(ContainSubstring("github-openstack-read-access"))
				Expect(prompt).To(ContainSubstring("Keep OpenStack follow-up read-only"))
				...
				Expect(prompt).To(ContainSubstring("If OpenStack lookup is only a required follow-up, phrase it as a next check"))
				...
				Expect(prompt).To(ContainSubstring("Do not mention OpenStack merely to say provider evidence was unavailable"))
			...
			It("should include a concrete compact example for step summary and Slack output", func() {
				...
				Expect(prompt).To(ContainSubstring("- *Network provisioning* (infra/external): Network creation failed with an OpenStack-backed quota or Neutron allocation signal; use read-only OpenStack diagnostics through github-openstack-read-access to confirm provider capacity before rerunning."))
