	...
	Context("When rendering input for AI analysis", func() {
		Describe("Given Claude is asked to write the report section", func() {
			It("should ask for pattern-level triage instead of repeated raw test lists", func() {
				...
				Expect(prompt).To(ContainSubstring("If Unikorn/Kubernetes CR observations are present, use them only as supporting evidence"))
				...
				Expect(prompt).To(ContainSubstring("When a Grafana signal is directly relevant and concrete"))
				Expect(prompt).To(ContainSubstring("When a CR signal is present, mention the concrete signal"))
				...
				Expect(prompt).To(ContainSubstring("Use Grafana-backed Slack bullets only when the Grafana observation directly supports the failure interpretation"))
				Expect(prompt).To(ContainSubstring("For Grafana-backed bullets, explicitly connect the test error"))
				Expect(prompt).To(ContainSubstring("For CR-backed bullets, explicitly connect the test error"))
				...
				Expect(prompt).To(ContainSubstring("Do not mention Grafana in Slack just to say a match was weak, time-disjoint, identifier-unmatched, or likely unrelated"))
				Expect(prompt).To(ContainSubstring("If Grafana only returned audit/cleanup rows, mention it in Slack only when those rows directly match"))
				...
				Expect(prompt).To(ContainSubstring("Do not add standalone supporting bullets such as '- *Evidence:*'"))
