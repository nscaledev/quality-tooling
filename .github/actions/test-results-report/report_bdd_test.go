	...
	Context("When preparing paths in the composite action", func() {
		Describe("Given raw path inputs are provided by GitHub", func() {
			...
			It("should use the Teleport application tunnel for optional Grafana MCP enrichment", func() {
				...
				Expect(action).To(ContainSubstring("GRAFANA_ORG_ID=${INPUT_GRAFANA_ORG_ID}"))
	...
	Context("When rendering input for AI analysis", func() {
		Describe("Given Claude is asked to write the report section", func() {
			It("should ask for pattern-level triage instead of repeated raw test lists", func() {
				...
				Expect(prompt).To(ContainSubstring("If Grafana/Loki observations are present, use them only as supporting evidence"))
				Expect(prompt).To(ContainSubstring("Keep the report close to the existing production format"))
				Expect(prompt).To(ContainSubstring("do not add a separate Grafana/Loki section"))
				Expect(prompt).To(ContainSubstring("When a Loki signal is present, mention the concrete signal"))
				...
				Expect(prompt).To(ContainSubstring("For Grafana/Loki-backed bullets, explicitly connect the test error"))
				Expect(prompt).To(ContainSubstring(`Do not use vague phrases like "Grafana returned related activity"`))
				Expect(prompt).To(ContainSubstring("If Loki only returned audit/cleanup rows"))
			...
			It("should include a concrete compact example for step summary and Slack output", func() {
				...
				Expect(prompt).To(ContainSubstring("- *File Storage attachment network* (infra/external): The test failed because network provisioning reached error instead of provisioned; Loki matched the resource only in audit/cleanup rows"))
