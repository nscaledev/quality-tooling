	...
	Context("When rendering input for AI analysis", func() {
		Describe("Given Claude is asked to write the report section", func() {
			...
			It("should include a concrete compact example for step summary and Slack output", func() {
				...
				Expect(prompt).To(ContainSubstring("- *File Storage attachment network* (infra/external): The test failed because network provisioning reached error instead of provisioned; Grafana matched the resource only in audit/cleanup rows"))
