...
func TestClaudePromptRequestsPatternSummary(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"Use Grafana observations, CR observations, and OpenStack observations only as supporting evidence",
		"Combine suite evidence, Grafana observations, CR observations, and OpenStack observations",
		...
		"github-openstack-read-access",
		"Keep OpenStack follow-up read-only",
		...
		"If OpenStack lookup is only a required follow-up, phrase it as a next check",
		...
		"Do not mention OpenStack merely to say provider evidence was unavailable",
...
func TestGrafanaLogQueryPlanningPromptRequestsBackendOnlyJSON(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"For OpenStack-backed provisioning failures, prefer queries that can find controller logs containing Neutron, Nova, Cinder, Keystone",
...
func TestUnikornCRQueryPlanningPromptRequestsReadOnlyCRJSON(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"Provider-backed OpenStack CR names or IDs",
		"For OpenStack-backed allocation or quota failures",
