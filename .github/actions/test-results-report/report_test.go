...
func TestClaudePromptRequestsPatternSummary(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"When a Grafana signal is directly relevant and concrete",
		...
		"Use Grafana-backed Slack bullets only when the Grafana observation directly supports the failure interpretation",
		...
		"Do not mention Grafana in Slack just to say a match was weak, time-disjoint, identifier-unmatched, or likely unrelated",
		"If Grafana only returned audit/cleanup rows, mention it in Slack only when those rows directly match",
