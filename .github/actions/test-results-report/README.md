...
`enable-test-history-log-lookup` is a separate opt-in that uses Grafana MCP to fetch previous failed `test_history` records from O11y. It does not fetch all test runs. The reporter builds one targeted Loki query per selected current failed test, scoped by `test-history-log-selector` (default `service_name="test-results-report"`), `test_history result failed`, the current test name, and excluding the current GitHub run ID when available. By default these lookups open a separate O11y Grafana MCP target (`test-history-log-grafana-app`, default `wo11y-grafana-dev`) and use the `product-loki` datasource instead of the environment `Loki` datasource used for backend/controller log enrichment. Returned records are filtered by repo, environment, suite, test ID/name, and failure fingerprint when those fields are present.
...
| `test-history-log-grafana-app` | No | `wo11y-grafana-dev` | Teleport Grafana app used for previous failed test-history record lookups |
| `test-history-log-grafana-url` | No | empty | Direct Grafana URL for previous failed test-history record lookups |
| `test-history-log-grafana-mcp-endpoint` | No | empty | Existing mcp-grafana endpoint for previous failed test-history record lookups |
| `test-history-log-grafana-tunnel-port` | No | `3001` | Local port used for the test-history Grafana Teleport tunnel |
| `test-history-log-grafana-mcp-port` | No | `8001` | Local streamable HTTP port used for the test-history mcp-grafana server |
